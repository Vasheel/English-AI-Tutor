-- Fix security issues with user data access
...
-- 2. Fix word_scramble_performance table security
-- Enable RLS on word_scramble_performance table
ALTER TABLE public.word_scramble_performance ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for word_scramble_performance
-- Users can only view their own performance data
CREATE POLICY "Users can view own performance only" 
ON public.word_scramble_performance 
FOR SELECT 
TO authenticated 
USING (user_id = auth.uid());

-- Note: word_scramble_performance is a view, so INSERT/UPDATE/DELETE policies are not needed
-- The underlying word_scramble_analytics table should also be secured

-- 3. Ensure word_scramble_analytics table is properly secured
-- Check if RLS policies exist and are proper
...
-- 4. Ensure all user-related tables have proper RLS
-- Check activity_sessions table policies
