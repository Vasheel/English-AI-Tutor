...
A working FastAPI server to demonstrate rate limiting
"""
...
@app.get("/")
def root():
    return {"message": "Rate Limiting Demo API", "status": "running"}
...
@limiter.limit("5/minute")  # Very low limit for testing
async def evaluate_grammar(request: Request, grammar_request: GrammarRequest):
    """Grammar evaluation with rate limiting - 5 requests per minute"""
    try:
...
@limiter.limit("10/minute")  # Higher limit for testing
def list_images(request: Request):
    """List available images with rate limiting - 10 requests per minute"""
    return [
...
def get_rate_limit_status(request: Request):
    """Get current rate limit status - 2 requests per minute"""
    return {
        "message": "Rate limiting is active",
        "limits": {
            "grammar_evaluation": "5/minute",
            "image_requests": "10/minute",
            "status_checks": "2/minute"
        },
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting Simple Rate Limiting Demo Server...")
    print("📊 Rate limits configured:")
    print("   - Grammar evaluation: 5/minute")
    print("   - Image requests: 10/minute")
    print("   - Status checks: 2/minute")
